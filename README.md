...
- Spotify mood detection
  ...
  - both of them are combined according this formula: audio_score\*0.6 + lyrics_score\*0.4
- User book upload & analysis
  - only read books with more than a 3 star review are selected
  - for each book it is retrieved the description
  - an analysis is done and for each book there are six emotions 'anger', 'disgust', 'fear', 'joy', 'neutral', 'sadness', 'surprise' with the relative scores
    - for the analysis are used two different models, one that supports only english books and another for books in other languages
  - at the end the text embedding are computed for the user books
...
git clone https://github.com/MariaPalcau/MoodRecs
cd FlaskProject
